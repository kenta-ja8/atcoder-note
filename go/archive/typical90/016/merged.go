...
func main() {
	...
	dbg("S: ", S, "N: ", N)
	answer := math.MaxInt
	for quotient1 := N / S[0]; quotient1 >= 0; quotient1-- {
		remainder1 := N - (S[0] * quotient1)
		for quotient2 := remainder1 / S[1]; quotient2 >= 0; quotient2-- {
			remainder2 := remainder1 - (S[1] * quotient2)
			quotient3 := remainder2 / S[2]
			remainder3 := remainder2 - (S[2] * quotient3)
			if remainder3 == 0 {
				answer = int(math.Min(float64(answer), float64(quotient1+quotient2+quotient3)))
			}
		}
	}
	out(answer)
